...
Matching::Matching()
    :local_map_ptr_(new CloudData::CLOUD()),
     ...
     pcd1(new CloudData::CLOUD()),
     pcd2(new CloudData::CLOUD()),
     pcd_merge(new CloudData::CLOUD()){
...
bool Matching::InitGlobalMap() {
    ...
    has_new_global_map_ = true;
     ...
     // pcl::io::loadPCDFile("/home/max/NiuSlam_0108/src/MappingLocalization/lidar_localization/slam_data/map/ChargePort_v1.pcd",*pcd1);
    // pcl::io::loadPCDFile("/home/max/NiuSlam_0108/src/MappingLocalization/lidar_localization/slam_data/map/garage_v1.pcd",*pcd2);
    // *pcd_merge += *pcd1;
    // *pcd_merge += *pcd2;
    // pcl::io::savePCDFileASCII("/home/max/NiuSlam_0108/src/MappingLocalization/lidar_localization/slam_data/map/garage_chargeport.pcd", *pcd_merge);
    return true;
}
...
bool Matching::ResetLocalMap(float x, float y, float z) {
    ...
    LOG(INFO) << "local map size:" << local_map_ptr_->points.size();
...
bool Matching::Update(const CloudData& cloud_data, Eigen::Matrix4f& cloud_pose) {
    ...
    std::vector<float> edge = box_filter_ptr_->GetEdge();
    for (int i = 0; i < 3; i++) {
        if (fabs(cloud_pose(i, 3) - edge.at(2 * i)) > 30.0 &&
            fabs(cloud_pose(i, 3) - edge.at(2 * i + 1)) > 30.0)
            continue;
